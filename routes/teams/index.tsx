...
export default function TeamsPage(
  { data: { groupedTeams, conferenceOrder, divisionOrder } }: PageProps<
    TeamsPageData
  >,
) {
            ...
            <div key={conference} class="flex flex-col">
              ...
              <div class="flex-1 grid grid-rows-3 gap-6">
                {divisionOrder[conference].map((division) => (
                  <div
                    key={division}
                    class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow duration-200 h-full"
                  >
                    <h3 class="text-xl font-semibold mb-4 text-gray-800">
                      {division} Division
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 auto-rows-fr">
                      {groupedTeams[conference]?.[division]?.map((
                        team: Team,
                      ) => (
                        <a
                          key={team.id}
